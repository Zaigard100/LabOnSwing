...
public class TransformRomb extends JDialog {
    ...
    public TransformRomb() {
        ...
        edit.addActionListener(e -> {
            ...
            if (Objects.equals(romb_list.getSelectedItem(), "all")) {
                ...
                switch (Objects.requireNonNull(item)) {
                    case "move":
                        Utils.getRombs().get(romb_list.getSelectedIndex()).move(Lab3.parseInt(field1.getText(), 0), Lab3.parseInt(field2.getText(), 0));
                        break;
                    case "x":
                        Utils.getRombs().get(romb_list.getSelectedIndex()).setX(Lab3.parseInt(field1.getText(), Utils.getRombs().get((romb_list.getSelectedIndex())).getX()));
                        break;
                    case "y":
                        Utils.getRombs().get(romb_list.getSelectedIndex()).setY(Lab3.parseInt(field1.getText(), Utils.getRombs().get((romb_list.getSelectedIndex())).getY()));
                        break;
                    case "fill":
                        Utils.getRombs().get(romb_list.getSelectedIndex()).setFill(checkBox.isSelected());
                        break;
                    case "size":
                        Utils.getRombs().get(romb_list.getSelectedIndex()).setWidth(Lab3.parseInt(field1.getText(), Utils.getRombs().get((romb_list.getSelectedIndex())).getWidth()));
                        Utils.getRombs().get(romb_list.getSelectedIndex()).setHeight(Lab3.parseInt(field2.getText(), Utils.getRombs().get((romb_list.getSelectedIndex())).getHeight()));
                        break;
                    case "color":
                        Utils.getRombs().get(romb_list.getSelectedIndex()).setColor(
                                Lab3.parseInt(field1.getText(), Utils.getRombs().get((romb_list.getSelectedIndex())).getColor().getRed()),
                                Lab3.parseInt(field2.getText(), Utils.getRombs().get((romb_list.getSelectedIndex())).getColor().getGreen()),
                                Lab3.parseInt(field3.getText(), Utils.getRombs().get((romb_list.getSelectedIndex())).getColor().getBlue())
                        );
                        break;
                }
            }
    ...
    public void reload() {
        romb_list.removeAllItems();
        for (int i = 0; i < Utils.getCircles().size(); i++) {
            romb_list.addItem(i+"");
        }
        romb_list.addItem("all");
    }
